...
SUMMARY_OUTPUT_FILE = '/Workspace/Users/jwang77@optumcloud.com/gpd-persona-ai-model-api/data/s-learning-data/eval/042025/summary_eval_results_0401_2025_0420_2025.csv'
...
def prepare_evaluation_features(behavioral_df, plan_df):
    ...
    # Check for columns needed in assign_quality_level
    required_cols = ['plan_id', 'dce_click_count', 'pro_click_count']
    ...
    print(f"Quality levels assigned: {training_df['quality_level'].value_counts().to_dict()}")
...
def evaluate_model(model, X, y_true, metadata):
    ...
    valid_mask = y_true.notna() & (~y_true.str.lower().isin(invalid_personas))
    ...
    print(f"\nOverall Accuracy (valid personas only): {overall_accuracy * 100:.2f}%")
    ...
    for persona in sorted(set(y_true_valid)):
        mask = y_true_valid == persona
        if mask.sum() > 0:
            persona_accuracy = accuracy_score(y_true_valid[mask], y_pred_valid[mask])
            print(f"Accuracy for '{persona}': {persona_accuracy * 100:.2f}% (Count: {mask.sum()})")

    # Breakdown by quality level
    for quality in ['High', 'Medium', 'Low']:
        mask = metadata_valid['quality_level'] == quality
        if mask.sum() > 0:
            y_true_quality = y_true_valid[mask]
            y_pred_quality = y_pred_valid[mask.values]
    ...
    cm_df = pd.DataFrame(cm, index=list(sorted(set(y_true_valid))), columns=list(sorted(set(y_true_valid))))
    print("\nConfusion Matrix (Overall, valid personas only):")
    print(cm_df)

    # Classification report
    print("\nClassification Report:")
    print(classification_report(y_true_valid, y_pred_valid, labels=list(sorted(set(y_true_valid))), target_names=list(sorted(set(y_true_valid)))))
